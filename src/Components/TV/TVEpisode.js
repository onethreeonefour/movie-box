...
function TVEpisode(props) {
  ...
  useEffect(() => {
    if (props.match.params.id) {
      fetch(
        `${API_URL}tv/${props.match.params.id}/season/${props.match.params.seasonNumber}/episode/${props.match.params.episodeNumber}?api_key=${API_KEY}`
      )
        .then((res) => res.json())
        .then((res) => {
          setDetail(res);
      ...
      fetch(
        `${API_URL}tv/${props.match.params.id}/season/${props.match.params.seasonNumber}/episode/${props.match.params.episodeNumber}/images?api_key=${API_KEY}`
      )
        .then((res) => res.json())
        .then((res) => {
          setImages(res);
  ...
  return (
    <div>
      {Detail ? (
        ...
        >
          <div className="heading-grid">
            <div>
              {Detail.still_path !== null ? (
                <img src={`${IMAGE_URL}/w342${Detail.still_path}`} alt="poster" />
              ) : (
                <img src={Blank} alt="poster" />
              )}
            </div>
            <div>
              ...
              <h4>
                User Score: <span className="user-score">{Detail.vote_average.toFixed(1)}</span> ({Detail.vote_count}{" "}
                voted)
              </h4>
            </div>
          </div>
      ...
      {Detail && Detail.guest_stars.length > 0 ? (
        <div className="tv-episode-wrapper">
      ...
      {Detail && Images ? (
        <div className="tv-episode-images-wrapper">
