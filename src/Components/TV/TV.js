          ...
          <div className="heading-grid">
            ...
            <div>
              ...
              {TV.created_by.length > 0 ? (
                <h4>
                  Created By <Link to={`/person/${TV.created_by[0].id}`}>{TV.created_by[0].name}</Link>
                </h4>
              ) : (
                <span></span>
              )}
              <h4>
                User Score: <span className="user-score">{TV.vote_average}</span> ({TV.vote_count} voted)
              </h4>
        ...
        <div className="season-container">
          {TV ? (
            TV.seasons.map((season, index) => {
              return (
                <a href={`/tv/${props.match.params.id}/${TV.name}/season/${season.season_number}`} key={index}>
                  <div className="season-card">
                    <h2>{season.name}</h2>
                    {season.poster_path === null ? (
                      <img src={Blank} alt="season"></img>
                    ) : (
                      <img src={`${IMAGE_URL}/w342${season.poster_path}`} alt="season"></img>
                    )}
                  </div>
                </a>
              );
